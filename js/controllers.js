...
.controller('menuCtrl', function ($scope, $http, sharedCartService, sharedFilterService) {
	...
	$scope.showProductInfo = function (id, desc, img, name,
		price) {
...
.controller('cartCtrl', function ($scope, $http, sharedCartService, $ionicPopup, $state) {
	...
	$scope.submitCart = function () {
		var confirmPopup = $ionicPopup.confirm({
			...
			.then(function (res) {
				if (res) {
					$scope.loggedin_id = sessionStorage
						.getItem('loggedin_id');
					$scope.loggedin_name = sessionStorage
						.getItem('loggedin_name');
					$scope.loggedin_phone = sessionStorage
						.getItem('loggedin_phone');
					$scope.loggedin_address = sessionStorage
						.getItem('loggedin_address');

					var products = [];
					for (var i = 0; i < sharedCartService.cart.length; i++) {
						products
						.push(sharedCartService.cart[i])
					}

					$http
					.get(
						'http://iligtas.ph/hotshots/checkout.php?'
						 + 'name='
						 + $scope.loggedin_name
						 + '&contact='
						...
						.stringify(products)
						 + '&total='
						 + sharedCartService.total_amount)
					...
					.success(
						...
						var alertPopup = $ionicPopup
							.alert({
								...
								template: '<center>Checkout Total: <b>'
								 + sharedCartService.total_amount
								 + '</b></center>'
							});
...
.controller('checkOutCtrl', function ($scope, sharedCartService, $http, $ionicPopup) {
	$scope.validateLocation = function (addressToValidate) {
		...
		function findLocation(xml) {
			...
			for (var i = 0; i < x.length; i++) {
				...
				if (matchRuleShort(addressToValidate, "*"
						 + location + "*") == true) {
	...
	$scope.checkOut = function (name, phone, address, e) {
		if (name == undefined || name.length == 0) {
			...
			$http
			.get(
				'http://iligtas.ph/hotshots/checkout.php?name='
				 + name
				 + '&contact='
				 + phone
				 + '&address='
			...
			.success(
				...
				$scope.hasCheckedOut = false;
				document
				.getElementById('coname').value = '';
				document
				.getElementById('conumber').value = '';
				document
				.getElementById('coaddress').value = '';
			});
		}
	}
})
...
.controller('loginCtrl', function ($scope, $http, $ionicPopup, $state, $ionicHistory) {
	...
	$scope.login = function () {
		auth_url = "http://iligtas.ph/hotshots/user-details.php?e="
			 + $scope.user.email
			 + "&p="
			 + $scope.user.password;
		$http.get(auth_url).success(
			function (response) {
			$scope.user_details = response.records[0];
			sessionStorage.setItem('loggedin_id',
				$scope.user_details.u_id);
			sessionStorage.setItem('loggedin_name',
				$scope.user_details.u_name);
			sessionStorage.setItem('loggedin_phone',
				$scope.user_details.u_phone);
			sessionStorage.setItem('loggedin_address',
				$scope.user_details.u_address);
...
.controller('signupCtrl', function ($scope, $http, $ionicPopup, $state, $ionicHistory) {
	...
	$scope.signup = function (data) {
		if ($scope.data.email == undefined
			 || $scope.data.email.length == 0) {
			alert("Please enter a valid email address.")
		} else if ($scope.data.password == undefined
			 || $scope.data.password.length == 0) {
			alert("Please enter your phone number.")
		} else if ($scope.data.phone == null
			 || $scope.data.phone == undefined
			 || $scope.data.phone.length == 0) {
			alert("Please enter your phone number.")
		} else if ($scope.data.address == undefined
			 || $scope.data.address.length == 0) {
			alert("Please enter your address.")
		} else if (!$scope
			.validateLocation($scope.data.address)) {
			alert("Delivery service is unavailable at your location.")
		} else {
			var signup_url = "http://iligtas.ph/hotshots/signup.php";
			var signup_obj = "?un=" + data.email + "&ps="
				 + data.password + "&ph=" + data.phone
				 + "&add=" + data.address;
			$http
			.get(signup_url + signup_obj)
			.success(
				function (response) {
				if (response == "0") {
...
.controller('favoritesCtrl', function ($scope, $http, $ionicPopup, $state, $ionicHistory) {
	...
	$scope.feedback = function (fav) {
		if ($scope.fav.comment == undefined
			 || $scope.fav.comment.length == 0) {
			alert("Please enter your suggestion or comment.")
		} else {
			var feedback_url = "http://iligtas.ph/hotshots/feedback.php?id="
				 + sessionStorage.getItem('loggedin_id')
				 + "&fb=" + fav.comment;
			$http
			.get(feedback_url)
			.success(
...
.controller('productPageCtrl', function ($scope, sharedCartService) {
	...
	angular.element(document).ready(
		function () {
		$scope.id = sessionStorage
			.getItem('product_info_id');
		$scope.desc = sessionStorage
			.getItem('product_info_desc');
		$scope.img = "img/"
			 + sessionStorage
			.getItem('product_info_img')
			 + ".png";
		$scope.name = sessionStorage
			.getItem('product_info_name');
		$scope.price = sessionStorage
			.getItem('product_info_price');

		$scope.addToCart = function (id, image, name,
			price, $ionicPopup) {
			cart.add(id, image.split('img/').join("")
				.split('.png').join(""), name,
				price, 1);
		};
...
.controller('mapCtrl', function ($scope) {
	$scope.initMap = function () {
		...
		var center = new google.maps.LatLng(14.8203331,
				120.2811585);
		var mapOptions = {
			center: center,
			zoom: 16
		...
		var socket = io
			.connect('http://hotshots-middleware.herokuapp.com');
		...
		socket.on('server:Location', function (data) {
			console.log(data);
			currentPosition = new google.maps.LatLng(data.lat,
					data.long);
			if (marker) {
